...
A standard interface for escrow STO(Security Token Offering) process.

## Abstract

The following standard allows for the implementation of a standard API for Escrow STO process within smart contracts.

There are 2 types of tokens in this process.
  - Issuing token: Issuer offers security tokens and buyer can hold it as share.
  - Payable token: Buyer can pay with this token(ex: USDT) to hold Issuing token. So Issuing token's value is based on price of payable token

In initial STO process,
  - issuer offers initial supplies
  - number of buyers purchase the token with payable tokens

Until STO process is finalized(Success or Failed), there are following compliances and rights
  - Only limited buyers can hold the tokens
  - Each buyer could hold tokens in (min - max) range
  - Buyer can deposit funds(with payable token) and refund until STO process is success.
  - Issuing process can be failed when condition does not meet. All payed token should be return to buyers.
  - When issuing process is finalized, all payed token should be transferred to issuer and issuing tokens should be transferred to buyers wallet.
  - During escrow process, all balance(issuer & buyers) should be locked.
